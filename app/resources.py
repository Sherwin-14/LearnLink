...
class CourseListAPI(Resource):
    ...
    @ns.doc(security=authorizations)
...
class StudentListAPI(Resource):
    ...
    def post(self):
        print(ns.payload)
        student=Student(name=ns.payload["name"],course=Course(name=ns.payload["course"]))
...
class StudentAPI(Resource):
    ...
    def put(self,id):
        ...
        student.course_id=ns.payload["course"]
        db.session.commit()
        return student
...
class Register(Resource):    
    ...
    def post(self):
        user=User(userrname=ns.payload["userrname"],password_hash=generate_password_hash(ns.payload["password"]))
        db.session.add(user)
        db.session.commit()
        return user,201
...
class Login(Resource):    
    ...
    @ns.expect(login_model)
    def post(self):
        user=User.query.filter_by(userrname=ns.payload["userrname"]).first()
        if not user:
            return {"error":"user does not exist"},401
        if not  check_password_hash(user.password_hash,ns.payload["password"]):
            return {"error":"Incorrect password"},401
        return {"acess_token":create_access_token(user)}
